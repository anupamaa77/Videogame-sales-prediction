...
def validate_clean_data(csv_path="/home/anupamarai24128432/vgsales_project/data/cleaned_vgsales.csv"):
...
# After validation, save the cleaned DataFrame
    cleaned_csv_path = "/home/anupamarai24128432/vgsales_project/data/validated_vgsales.csv"
    df.to_csv(cleaned_csv_path, index=False)
    print(f"✅ Cleaned and validated data saved to {cleaned_csv_path}")

    # Return the cleaned DataFrame for use in the next notebook (if needed)
    return df

# Run validation and save the cleaned DataFrame to CSV
if __name__ == "__main__":
    cleaned_df = validate_clean_data()  # Capture the validated DataFrame
